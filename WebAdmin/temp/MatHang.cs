...
namespace CMS.DataAccess
{
	...
	public partial class MatHang : ActiveRecord<MatHang>, IActiveRecord
	{
		...
		public CMS.DataAccess.DonHangCollection GetDonHangCollection() { return MatHang.GetDonHangCollection(this.MaMH); }
		public static CMS.DataAccess.DonHangCollection GetDonHangCollection(Guid varMaMH)
		{
		    SubSonic.QueryCommand cmd = new SubSonic.QueryCommand("SELECT * FROM [dbo].[DonHang] INNER JOIN [CTDH] ON [DonHang].[MaDH] = [CTDH].[MaDH] WHERE [CTDH].[MaMH] = @MaMH", MatHang.Schema.Provider.Name);
			cmd.AddParameter("@MaMH", varMaMH, DbType.Guid);
			IDataReader rdr = SubSonic.DataService.GetReader(cmd);
			DonHangCollection coll = new DonHangCollection();
			coll.LoadAndCloseReader(rdr);
			return coll;
		}
		
		public static void SaveDonHangMap(Guid varMaMH, DonHangCollection items)
		{
			QueryCommandCollection coll = new SubSonic.QueryCommandCollection();
			//delete out the existing
			QueryCommand cmdDel = new QueryCommand("DELETE FROM [CTDH] WHERE [CTDH].[MaMH] = @MaMH", MatHang.Schema.Provider.Name);
			cmdDel.AddParameter("@MaMH", varMaMH, DbType.Guid);
			coll.Add(cmdDel);
			DataService.ExecuteTransaction(coll);
			foreach (DonHang item in items)
			{
				Ctdh varCtdh = new Ctdh();
				varCtdh.SetColumnValue("MaMH", varMaMH);
				varCtdh.SetColumnValue("MaDH", item.GetPrimaryKeyValue());
				varCtdh.Save();
			}
		}
		public static void SaveDonHangMap(Guid varMaMH, System.Web.UI.WebControls.ListItemCollection itemList) 
		{
			QueryCommandCollection coll = new SubSonic.QueryCommandCollection();
			//delete out the existing
			 QueryCommand cmdDel = new QueryCommand("DELETE FROM [CTDH] WHERE [CTDH].[MaMH] = @MaMH", MatHang.Schema.Provider.Name);
			...
			foreach (System.Web.UI.WebControls.ListItem l in itemList) 
			{
				if (l.Selected) 
				{
					Ctdh varCtdh = new Ctdh();
					varCtdh.SetColumnValue("MaMH", varMaMH);
					varCtdh.SetColumnValue("MaDH", l.Value);
					varCtdh.Save();
				}
			}
		}
		public static void SaveDonHangMap(Guid varMaMH , string[] itemList) 
		{
			QueryCommandCollection coll = new SubSonic.QueryCommandCollection();
			//delete out the existing
			 QueryCommand cmdDel = new QueryCommand("DELETE FROM [CTDH] WHERE [CTDH].[MaMH] = @MaMH", MatHang.Schema.Provider.Name);
			...
			foreach (string item in itemList) 
			{
				Ctdh varCtdh = new Ctdh();
				varCtdh.SetColumnValue("MaMH", varMaMH);
				varCtdh.SetColumnValue("MaDH", item);
				varCtdh.Save();
			}
		}
		
		public static void DeleteDonHangMap(Guid varMaMH) 
		{
			QueryCommand cmdDel = new QueryCommand("DELETE FROM [CTDH] WHERE [CTDH].[MaMH] = @MaMH", MatHang.Schema.Provider.Name);
			cmdDel.AddParameter("@MaMH", varMaMH, DbType.Guid);
